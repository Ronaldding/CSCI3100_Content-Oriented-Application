  ...
  try {
    ...
    if (userFollowings.length > 0) {
      ...
      if (followingsFollowings.length > 0) {
        const userRecommend = await User.find({
          _id: { $in: followingsFollowings },
        })
        return res.status(200).json(userRecommend)
      } else {
        const publicRecommend = await User.find({
          _id: { $ne: '660970232846199a041ae117' },
          isPublic: true,
        })
    ...
    const postRecommend = await Post.aggregate([
      {
        $match: {
          isHide: false,
          userId: { $in: publicUserIds.map((id) => id.toString()) },
        },
      },
