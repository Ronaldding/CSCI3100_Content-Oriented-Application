...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (post.userId === req.body.userId|| user.isAdmin) {//owner of the post
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (post.userId === req.body.userId || user.isAdmin) {
      ...
      res.status(403).json("You can delete only your post or you need admin privileges");
    }
  } catch (err) {
    res.status(500).json(err);
  }
...
router.get("/all/timeline", async (req, res) => {
  try {
    ...
    const timelinePosts = await Post.find({ userId: { $in: publicUserIds }, isHide: false });
